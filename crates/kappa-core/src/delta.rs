...
pub enum DeltaError {
    ...
    VarintOverflow,
    #[error("base size mismatch: expected {expected}, got {got}")]
    BaseSizeMismatch { expected: usize, got: usize },
    #[error("copy out of bounds: offset {offset}, size {size}, base len {base_len}")]
    CopyOutOfBounds {
        offset: usize,
        size: usize,
        base_len: usize,
    },
    #[error("reserved opcode 0x00")]
    ReservedOpcode,
    #[error("result size mismatch: expected {expected}, got {got}")]
    ResultSizeMismatch { expected: usize, got: usize },
}
...
pub fn decode_varint(data: &[u8], pos: &mut usize) -> Result<usize, DeltaError> {
...
pub fn apply_delta(base: &[u8], delta: &[u8]) -> Result<Vec<u8>, DeltaError> {
    ...
    while pos < delta.len() {
        ...
        if cmd & 0x80 != 0 {
            ...
            let n = cmd as usize;
            if pos + n > delta.len() {
                return Err(DeltaError::Truncated("insert data"));
            }
